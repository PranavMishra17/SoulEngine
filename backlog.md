...
| 5.20 | **The Speaker sees almost none of an NPC's memory.** `core/context.ts:625` builds the slim prompt with `formatMemories(instance, 2)` — one short-term and one long-term entry. With 13 STM and 1 LTM, SLOP told a player it had never met them. Raising the count trades prompt cost against continuity and is a product decision, not a silent edit: needs a chosen number, a token-budget check, and a cross-session test proving recall works after it. ERR-025. | M | 5.17 | reg | todo |
| 5.21 | **Voice turns are not written to the session log.** `src/voice/pipeline.ts` runs its own loop rather than `runConversationTurn`, so voice sessions record start and end but no turns. Either route voice through the shared turn function or add the same append at its seam. | M | 5.19 | e2e | todo |
...
| 5 | 20 | 10 | broker, eval harness, voice vending and streaming landed; Unity runs without provider keys; text harness in use and surfacing real defects |
| 6 | 8 | 0 | not started |
| **Total** | **82** | **48** | — |
