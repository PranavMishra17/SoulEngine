...
| # | Blocker | Evidence |
|---|---|---|
| ~~B1~~ | **FIXED 2026-09-05** — resolved by deleting `UseBackendProxy` along with the throwing branch. Original: **Default config throws on first conversation.** `UseBackendProxy` defaults `true`; `GetLLMConfig()` unconditionally throws in that mode; every caller invokes it unguarded. | `SoulEngineConfig.cs:49`, `:203-205`; `NPCConversationController.cs:127-128` |
| B2 | **Backend Proxy mode has no implementation.** No proxy provider class exists. `RuntimeLlmProvider`/`RuntimeTtsProvider` are populated and read by nothing. **RESOLVED 2026-09-05:** proxy mode was deleted rather than built. Broker-token mode shipped instead — the SDK fetches a short-lived credential from the developer's broker (§3.3 Option B). | grep for `proxy` returns only flags, tooltips, and doc comments |
...
| ~~B13~~ | **FIXED 2026-09-05** (`SoulEngine-Unity` commit `e6ac2e0`). Original: **SUBMISSION-BLOCKING. `SoulEngineConfig.asset` carries `LlmApiKey`/`MindApiKey`/`TtsApiKey`/`SttApiKey` as fields on a ScriptableObject that ships in the build.** Unity Submission Guidelines §1.5.b: *"Third-party API keys are not stored in ways that would incorporate the key into project builds (for example, inside any script or GameObject that would be included in a scene)."* The package would have failed submission. All four fields are deleted; the client now exchanges its Game Client API Key for a short-lived scoped broker credential (`Security/BrokerCredentialClient.cs`) and routes completions through `Providers/LLM/BrokerLLMProvider.cs`. Verified by headless compile: zero errors. | `SoulEngineConfig.cs:34` and the shipped `Assets/SoulEngineConfig.asset`; [`research/03-marketplace-and-multi-engine.md`](research/03-marketplace-and-multi-engine.md) §Q4 |
...
| D5 | Pricing model | **Answered 2026-09-05** — free asset, paid service, Verified Solution badge. The only AI-NPC asset on the Unity store with traction (Convai, 4.8/154) is free; every paid one shows "Not enough ratings". See [`research/07-pricing-and-comparables.md`](research/07-pricing-and-comparables.md). |
| D5b | Entitlement source | **Partly answered.** Unity's Invoice API is sanctioned by name (§1.4.a) but its endpoint/auth/response were not retrieved. **Fab and Godot have no seller entitlement API at all** — vendor-run gating is the only option there, and Fab cannot host a subscription. Still deferred by choice; model the seam. |
...
| D16 | Cognition runtime v2 — one streamed LLM call owning speech, tool calls and recall, behind §3.6 | **BUILT, NOT DEFAULT (2026-09-12).** Landed as `SingleCallRuntime` behind the §3.6 seam; measured equal on recall, one call, ~40% fewer input tokens, ~15% lower p50 wall — but gpt-4o and gemini-2.5-flash emit speech *or* a tool call in one persona-primed generation, never both, so it loses actions the parallel Mind performs. Default stays `parallel`. Next shape: a structured single call (backlog 7.16) where speech and action are fields of one required tool. Numbers: [`research/09-npc-runtime/BASELINE.md`](research/09-npc-runtime/BASELINE.md). |
