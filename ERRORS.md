...
|---|---|---|---|---|---|---|---|
| ERR-031 | P2 | core | `stripNarration` deleted emphasised words: a live reply "I remember *Kael* owes forty crowns" reached the player as "I remember  owes forty crowns" | `cleaned.replace(/\*[^*]*\*/g, '')` in `src/conversation/turn.ts` removed every asterisked span, stage direction or not | 7.15 | `tests/regression/err-031-strip-narration-keeps-emphasis.test.ts` | FIXED |
| ERR-030 | P1 | core | The NPC denies what the player said two turns earlier ("This is the first time we've spoken") in 4 of 5 trials on both Gemini and OpenAI, with the turn in its history and nothing deferred | `[RELATIONSHIP TO PLAYER]` renders "This is your first interaction with this person" (`core/context.ts:180-182`) and `[THE PERSON YOU'RE TALKING TO]` renders "Treat them as a stranger" (`context.ts:147-149`) for any player without a relationship record, which is every first-session player; the model obeys the system prompt over the conversation history. Found by the playground on its first live run, see `research/09-npc-runtime/BASELINE.md` | 7.13 | `tests/regression/err-030-first-session-not-first-meeting.test.ts` (wording) + `tests/fixtures/playground/deferred-recall-strict.json` (live gate, 5/5 on gpt-4o and gemini-2.5-flash after the fix, from 1/5 and 0/5) | FIXED |
...
| ERR-023 | P1 | storage | A rotated `ENCRYPTION_KEY` made every conversation in a project impossible, including ending one | `getSessionContext` and `endSession` both threw when stored provider keys could not be decrypted, though their own provider resolution already falls back to the configured global provider when a project has no key | 5.17 | `tests/regression/err-023-unreadable-keys.test.ts` (pending) | FIXED |
| ERR-024 | P0 | storage | Every player shared one NPC mind; NPCs sharing an id prefix shared one too | instance id was `base64url(npcId:playerId)` truncated to 12 chars = 9 bytes, consumed before reaching the colon, so the player id never entered the hash. `test-player`, `harness_player` and an empty player all yielded `inst_bnBjX21qeHFr` | 5.18 | `tests/regression/err-024-instance-id-collision.test.ts` | FIXED |
| ERR-025 | P1 | core | An NPC with 14 stored memories reaches its own conversation with one | the Speaker's slim prompt calls `formatMemories(instance, 2)` (`core/context.ts:625`), split 1 STM + 1 LTM; when the top STM and LTM are the same promoted entry the character sees a single memory, duplicated. Observed live: SLOP answered "I don't believe we've met before" to a player it held 13 memories about | 5.20 | `tests/regression/err-025-prompt-memory-selection.test.ts` | FIXED |
| ERR-029 | P0 | storage | In production every request without a signed-in user reaches for the local filesystem instead of Supabase, so it fails on a read-only container. Anonymous `GET /api/v1/projects` returns 500 `EACCES: permission denied, mkdir 'data/projects'`, and the same path takes every game-client request | `getStorage` (`storage/factory.ts:33`) and `getStorageForUser` (`storage/hybrid.ts:17`) both select the backend from `userId` alone and fall back to local when it is null. A game client authenticates with a project API key and carries no userId, so the entire runtime path -- start session, converse, end session -- selects local storage in production. The fallback is correct for local dev and for `isDevUserId`, but in a Supabase-backed deployment it can never be right. Storage must key off the authenticated principal, which may be a project key rather than a user. Unaffected locally, which is why it went unseen | 5.25 | `tests/regression/err-029-cloud-storage-selection.test.ts` | FIXED |
| ERR-028 | P1 | storage | `sql/07-session-and-integrity.sql` aborts halfway when run, so the session-persistence tables and `knowledge_categories.description` are never created | the file added a unique constraint as `UNIQUE (instance_id, version) NOT VALID`. Postgres accepts `NOT VALID` only for CHECK and FOREIGN KEY constraints, so it fails with `0A000: UNIQUE constraints cannot be marked NOT VALID` and takes the rest of the file with it. Replaced with `CREATE UNIQUE INDEX IF NOT EXISTS`, which is idempotent and raises the same SQLSTATE 23505 that `instances.ts:183` keys off. Nothing in the repo executes these files, so it surfaced only when a human pasted it into the SQL editor | - | `tests/regression/err-028-sql-unique-not-valid.test.ts` | FIXED |
| ERR-027 | P1 | core | Any short-lived process that ran a conversation turn never exited; `npm run eval` printed its whole report and then hung forever | `InMemoryCooldownStore` (`src/mcp/cooldown-store.ts:80`) starts a 60s sweeper in its constructor and never unrefs it, and `src/mcp/exit-handler.ts` constructs one at module load. A referenced interval keeps the event loop alive, so merely importing `src/conversation/turn.ts` pinned the process. The server never noticed because it runs indefinitely, and Vitest force-exits its workers | 5.19 | `tests/regression/err-027-cooldown-sweeper-unref.test.ts` | FIXED |
| ERR-026 | P1 | core | The summariser shredded the memories it was meant to protect, storing first-person notes with words and clauses spliced out | two generations of over-broad injection filtering in `core/summarizer.ts`. The first deleted everything between any two quote-like characters, apostrophes included, so contractions became splice points; replaying it turns "claiming we'd met before, but I couldn't recall them at all ... I'm not sure why they're here" into the stored "claiming wet recall them at all ... Ire here." The second matched bare English prefixes (`forget all`, `new instruction`) and deleted through to the next sentence terminator, turning "I won't forget all the trouble they caused me. They seemed honest enough." into "I won't . They seemed honest enough." Neither required the imperative form, and both deleted rather than redacted | 5.22 | `tests/regression/err-026-summary-injection-filter.test.ts` | FIXED |
