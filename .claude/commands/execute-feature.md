...
5. **Integrate + own the meta.** Merge the agent's worktree branch into the working branch (resolve conflicts; re-run `npm test` after merge). Then YOU — not the agent — update the shared trackers from its report: set the item `done` in `backlog.md`, flip its `ERRORS.md` row to `FIXED` with the test path + a one-line detail, and file any new bug it reported as a fresh `ERRORS.md`/`backlog.md` row. Commit the meta update separately with a clean message.
6. **Report to the user:** item, what changed, the new test(s), suite status, commit hash, and any follow-up items filed.
